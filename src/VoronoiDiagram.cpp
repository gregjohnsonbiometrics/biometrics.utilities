...
VoronoiDiagram::VoronoiDiagram(const std::vector<Vector2>& points, const std::vector<double> &weights )
{
    mSites.reserve(points.size());
    mFaces.reserve(points.size());
    for(std::size_t i = 0; i < points.size(); ++i)
    {
        mSites.push_back(VoronoiDiagram::Site{i, points[i], weights[i], nullptr});
...
bool VoronoiDiagram::intersect(Box box)
{
    ...
    for (const Site& site : mSites)
    {
        ...
        Box::Side incomingSide = Box::Side::BOTTOM, outgoingSide = Box::Side::LEFT;
