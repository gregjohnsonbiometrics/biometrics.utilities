...
Arc* Beachline::locateArcAbove(const Vector2& point, double l) const {
    ...
    while (!found) {
        ...
        if (!isNil(node->prev)) {
            breakpointLeft = computeBreakpoint(node->prev->site->point, node->prev->site->weight, node->site->point, node->site->weight, l);
        }

        if (!isNil(node->next)) {
            breakpointRight = computeBreakpoint(node->site->point, node->site->weight, node->next->site->point, node->next->site->weight, l);
        }
